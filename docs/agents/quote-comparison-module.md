...
- External Quote Import lets an estimator start Quote Comparison from client-provided quote files. The UI has one quote dropzone; the product path uses deterministic extraction/import and fails visibly with the deterministic error when it cannot parse priced quote rows, rather than privately retrying through GPT-5.5.
...
- `apps/web/lib/procurement/external-quote-import.ts` parses extracted quote text into a `ContractorRFQ` plus imported `ContractorBid`; any explicit developer/operator repair flow must re-enter this importer rather than bypass it.
