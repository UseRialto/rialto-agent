...
// ---------------------------------------------------------------------------
// Order reminder emails (sent by cron; no attachment)
// ---------------------------------------------------------------------------

function buildOrderReminderEmailBody(params: {
  vendorName: string
  poNumber: string
  rfqTitle: string
  ...
  const { vendorName, poNumber, rfqTitle, reminderIndex, expectedDeliveryDate, updateUrl } = params
  const deliveryLine = expectedDeliveryDate
    ? `Expected delivery: ${new Date(expectedDeliveryDate).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}`
    : ''

  const subject = `[Reminder ${reminderIndex}/4] Order ${poNumber} status update requested — ${rfqTitle}`
  const body = [
    `Hi ${vendorName || 'there'},`,
    '',
    `This is a friendly reminder that we are tracking PO ${poNumber} for "${rfqTitle}".`,
    deliveryLine,
    '',
    'Please take a moment to update the current status of this order so we can keep our project timeline accurate:',
    '',
    updateUrl,
    '',
    'The link above is unique to this reminder — you can use it without logging in.',
    '',
    'Thank you,',
    'Rialto Procurement',
...
export async function sendOrderReminderEmail(params: {
  contractorUserId: string
  vendorEmail: string
  vendorName: string
  poNumber: string
  ...
  const mailbox = await getMailboxRow(params.contractorUserId)
  if (!mailbox.email_address || !mailbox.refresh_token) {
    console.warn(`[order-reminders] Contractor ${params.contractorUserId} has no connected mailbox — skipping reminder for PO ${params.poNumber}`)
    return
  }

  const provider = mailboxProvider(mailbox)
  const updateUrl = `${params.baseUrl}/vendor/order-update/${params.rawToken}`
  const { subject, body } = buildOrderReminderEmailBody({
  ...
  const mime = buildReplyMimeMessage({
    fromEmail: mailbox.email_address,
    fromName: mailbox.sender_name || 'Rialto Procurement',
    toEmail: params.vendorEmail,
    toName: params.vendorName || params.vendorEmail,
    subject,
    body,
  })

  if (provider === 'microsoft_365') {
    await sendMicrosoftSimpleMessage(params.contractorUserId, {
      subject,
      htmlBody: mime.htmlBody,
      toEmail: params.vendorEmail,
      toName: params.vendorName || params.vendorEmail,
    ...
    await sendGmailMessage(params.contractorUserId, mime.raw)
  }
}
