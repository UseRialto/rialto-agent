...
function assertImportedBid(
  ...
  for (const expectedLine of expected.lines) {
    ...
    if (actual.unit_price !== expectedLine.unitPrice) mismatches.push(`${prefix}: unit price ${actual.unit_price} != ${expectedLine.unitPrice}`)
    if (actual.total_price !== expectedLine.totalPrice) mismatches.push(`${prefix}: total ${actual.total_price} != ${expectedLine.totalPrice}`)
