...
export const metadata = { title: 'Create RFQ - Rialto' }
...
export default async function CreateRFQPage({
  ...
  const initialTitle = existingRFQ?.title ?? `${project.name} - ${new Date().toLocaleString('en-US', { month: 'long' })} ${new Date().getFullYear()}`
    ...
    <div className="mx-auto w-full max-w-[88rem] px-8 pb-10 sm:px-10 lg:px-14">
      <div className="mb-7">
        <nav className="mb-3 flex items-center gap-1.5 text-xs" style={{ color: '#8a9e96' }}>
          ...
          <span>/</span>
          <span className="font-semibold" style={{ color: '#1e3a2f' }}>{existingRFQ ? 'Edit RFQ Draft' : 'New RFQ'}</span>
        </nav>
        <h1 className="text-2xl font-bold tracking-tight" style={{ fontFamily: 'var(--font-lora, Georgia, serif)', color: '#1e3a2f' }}>
          {existingRFQ ? 'Edit RFQ Draft' : 'Create an RFQ'}
        </h1>
        <p className="mt-1 text-sm" style={{ color: '#4a6358' }}>
          {existingRFQ
            ? 'Update your draft RFQ, revise vendors, and publish when the package is ready.'
            : 'Build an exact-price RFQ, invite vendors, and publish to start receiving bids.'}
        </p>
      </div>
