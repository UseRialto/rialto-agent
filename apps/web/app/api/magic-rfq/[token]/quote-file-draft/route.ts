...
export async function POST(request: Request, context: { params: Promise<{ token: string }> }) {
  ...
  if (access.status !== 'ok' || !access.rfq) {
    return NextResponse.json({ success: false, error: 'This magic form link is no longer valid.' }, { status: 401 })
  }
  ...
  try {
    ...
    if (uploadedFile) {
      ...
      source = {
        kind: 'inline_text',
        filename: String(formData.get('filename') ?? 'inline-email-reply.txt'),
        text: bodyText,
      }
    } else {
    ...
    const draft = await buildVendorMagicLinkQuotePrefill({
      rfq: access.rfq,
    ...
    console.error('Magic RFQ quote file draft failed:', error)
    return NextResponse.json(
